...
class Kmgio :
    ...
    def __lt__(self, other): # Ottima idea
...
def Anno_acq(nomeFile) :
    ...
    while True :
        ...
        if not line :            
            break                 
        ls = line.split() #inserisci 'ls' direttamente in 'l', non necessario usare 'l_kmgio' e un secondo for
...
def main() :
    ...
    l = Anno_acq(nomeFile)   
    print(l) # Visualizzazione del file. Per non stampare 'None' bisognava usare 2 'for' con un 'if' che controllasse l[i][j] != None

    ''' Non so come usare questo metodo
    g = []
    for el in l :
        g.append(el.sett)
